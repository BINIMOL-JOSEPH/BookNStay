...
def sent_confirmation_mail(to_email, link, name):
    ...
    message = format_html(
        'Hi {name},<br><br>'
        'You registered an account on BooknStay. '
        'Your account has been verified and activated. You are ready to login to the application with your registered email id and password<br><br>'
        '<center>'
            '<a href="{link}" style="display: inline-block; padding: 10px 20px; background-color: #3498db; color: #ffffff; text-decoration: none; border-radius: 5px;">Login</a>'
        '</center><br><br>'
        'If the button above does not work, you can also click on the following link: {link} <br><br><br>Thanks & regards, <br>BooknStay Team',
        name=name,
        link=link
    )
...
def send_rejection_mail(to_email, name):
    ...
    message = format_html(
        'Hi {name},<br><br>'
        'We regret to inform you that your registration request on BooknStay has been rejected.<br><br>'

        'Thanks & regards, <br>BooknStay Team',
        name=name,
    )
    from_mail = settings.EMAIL_HOST_USER
    recipient_list = [to_email]
...
class ListHotelRoomDetails(APIView):
    def get(self, request, hotel_id, format=None):
        try:
            ...
            room_data = self.get_room_data(request, room_details)
    ...
    def get_room_data(self, request, room_details):
        room_data = []
...
class CustomerReservationList(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not customer_id:
            return Response({'message': 'Unknown user.'}, status=status.HTTP_400_BAD_REQUEST)
        # status_filter = request.data.get['data']['status_filter']
        ...
        try:
            ...
            # query = request.data['params']['query']
            print('Paramas : ', query)
...
class GetSelectedWeekBookings(APIView):
    ...
    def get_queryset(self):
        ...
        start_day_of_week = start_date.weekday()  # Zero-based indexing
        ...
        for day_of_week in range(7):  # Loop over 0 to 6
...
class UnreadHotelNotificationCount(APIView):
    ...
    def get(self, request):
        unread_count = Notification.objects.filter( is_hotel_read=False).count()
        return Response({"unread_count": unread_count}) 

class HotelLocationAPI(APIView):
    
    def get(self, request):
        locations = Hotel.objects.values_list('city', flat=True).distinct()
...
class GetRoomDetails(APIView):
    ...
    def get_room_images(self, request, room_details_id):
        room_images = RoomImages.objects.filter(room_details_id=room_details_id).first()

        base_url = request.build_absolute_uri('/')[:-1]
        return {
            'image1': f"{base_url}{room_images.image1.url}" if room_images and room_images.image1 else None,
            'image2': f"{base_url}{room_images.image2.url}" if room_images and room_images.image2 else None,
            'image3': f"{base_url}{room_images.image3.url}" if room_images and room_images.image3 else None
        }
    ...
    def get(self, request, room_details_id=None, format=None):
        try:
            ...
            room_details_serializer = RoomDetailsSerializer(room_details)
            room_details_data = room_details_serializer.data
            room_images_data = self.get_room_images(request, room_details_id)
