...
def sent_verification_mail(to_email, link, name):
    ...
    message = format_html(
        ...
        'Before being able to use your account, you need to verify that this is your email address by clicking the button below:<br><br>'
        '<center>'
            '<a href="{link}" style="display: inline-block; padding: 10px 20px; background-color: #3498db; color: #ffffff; text-decoration: none; border-radius: 5px;">Verify Email</a>'
        '</center><br><br>'
...
class UnreadCustomerNotificationCount(APIView):
    ...
    def get(self, request):
        unread_count = Notification.objects.filter( is_customer_read=False).count()
